...
export class OrthSnaplineImpl extends View implements IDisablable {
  public readonly options: OrthSnaplineImpl.Options
  ...
  constructor(options: OrthSnaplineImpl.Options & { graph: Graph }) {
    super()
      ...
      if (view && view.cell.isNode()) {
        ...
        if (horizontalTop != null || verticalLeft != null) {
          ...
          if (horizontalTop != null) {
            dy =
              trueDirection.indexOf('bottom') !== -1 ? horizontalTop - nodeBottomRight.y : nodeTopLeft.y - horizontalTop
          }
        }
        ...
        if (angle % 90 === 0) {
          ...
          const quadrant =
            angle >= 0 && angle < 90 ? 1 : angle >= 90 && angle < 180 ? 4 : angle >= 180 && angle < 270 ? 3 : 2
  ...
  protected update(metadata: {
    ...
    if (metadata.horizontalTop) {
      ...
      const end = this.graph.localToGraph(
        new Point(metadata.horizontalLeft! + metadata.horizontalWidth!, metadata.horizontalTop)
      )
    ...
    if (metadata.verticalLeft) {
      ...
      const end = this.graph.localToGraph(
        new Point(metadata.verticalLeft, metadata.verticalTop! + metadata.verticalHeight!)
      )
...
export namespace OrthSnaplineImpl {
